...
static int prompt_otp(pam_handle_t *pamh, char *otp_code, size_t otp_len) {
    ...
    /* Copy response */
    strncpy(otp_code, resp->resp, otp_len - 1);
    otp_code[otp_len - 1] = '\0';
...
/* Inform user about push notification */
...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc, const char **argv) {
    ...
    if (config.auth_methods & AUTH_METHOD_PUSH) {
    ...
    /* Try OTP if enabled and push failed or not enabled */
    if (config.auth_methods & AUTH_METHOD_OTP) {
